<template>
  <el-card>
    <template #header>
      ...
      <el-button v-else plain type="primary" @click="onAdd" v-loading.fullscreen.lock="state.fullscreenLoading">
        <el-icon>
          <DocumentAdd />
        </el-icon>
        <span>新增</span>
      </el-button>
      <el-button plain type="primary" @click="state.dialogFormVisible = !state.dialogFormVisible">
        <el-icon>
          <Search />
        </el-icon>
        <span>查询</span>
      </el-button>
      <el-button v-if="state.queryMode" plain type="danger" @click="onRepeal" v-loading.fullscreen.lock="state.fullscreenLoading">
        <el-icon>
          <Delete />
        </el-icon>
        <span>資料作廢</span>
    ...
    <el-form label-width="120px" label-position="left">
      ...
      <el-form-item label="產品型號">
        <el-input v-model="sharedData.form.productNo" :disabled="state.queryMode"/>
      </el-form-item>
...
watch(() => props.slider, async (newVal) => {
  queryForm.dataIndex = newVal - 1
  await queryData()
})

watch(() => props.sharedData.change, async (newVal) => {
